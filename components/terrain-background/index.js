...
// Pre-calculated random pool for high-performance stable sketching wobbly lines
...
export default function TerrainBackground() {
  ...
  // High-performance refs to prevent re-instantiating loops or re-rendering on tick
  ...
  // Sync theme changes instantly into anim loop context
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // 2. Track global cursor coordinates for topographic peak distortion
    ...
    // 3. Tab Visibility checking to throttle loop when page is hidden
    const handleVisibilityChange = () => {
      isTabVisibleRef.current = !document.hidden;
    };
    document.addEventListener("visibilitychange", handleVisibilityChange);

    // 4. Initialize skeuomorphic sky elements
    // Birds drifting across the parchment
    const birds = [
    ...
    // Constellations for dark charcoal mode
    ...
    // Helper: Draw sketchy wobbly stroke path to simulate natural graphite pencil lines
    const drawWobblyLine = (pts, color, width) => {
      ...
      for (let i = 1; i < pts.length; i++) {
        ...
        // Draw wobbly segment midpoints for lines longer than 8px
        if (distSq > 64) {
    ...
    // Color gradient mapping helper based on layer depth (t: 0 at back to 1 at front)
    const getThemeColorAt = (t, isDark) => {
      let r, g, b;
      if (isDark) {
        // Refined graphite/navy wave layers
        if (t < 0.35) {
          const localT = t / 0.35;
          r = 14 + 10 * localT;
          g = 21 + 13 * localT;
        ...
        // Sepia light mode layers
        // Back: Tan base (221, 216, 196) -> Mid: Rich Coffee (212, 163, 115) -> Front: Dark Charcoal (60, 42, 33)
        if (t < 0.35) {
          const localT = t / 0.35;
          r = 221 - (221 - 212) * localT;
          g = 216 - (216 - 163) * localT;
          b = 196 - (196 - 115) * localT;
        } else if (t < 0.65) {
          const localT = (t - 0.35) / 0.3;
          r = 212 - (212 - 60) * localT;
          g = 163 - (163 - 42) * localT;
    ...
    const tick = () => {
      // Pause updates completely if page tab is inactive (resource saver)
      ...
      // Reset static random pool index for frame-level drawing stability (removes jittering TV static look)
      poolIdx = 0;

      // Increment clock for sine waves
      ...
      // Responsive scale settings: throttle layer counts and step densities for buttery-smooth mobile execution
      ...
      // Off-center celestial body positioning (Sun or Moon)
      ...
      for (let layer = 0; layer < layersCount; layer++) {
        ...
        for (let s = 0; s <= steps; s++) {
          ...
          // Main sine wave math (interference loops)
          let wave = Math.sin(tx * Math.PI * 4 + time * 2 + layer * 0.5) * 18 * (1 - tLayer * 0.4);
          wave += Math.cos(tx * Math.PI * 10 - time + layer) * 5;

          // Central Topographic Mountain Peak (Gaussian distribution centered on 0.5)
        ...
        // Draw topographic isolines (dashed on every 3rd contour layer)
